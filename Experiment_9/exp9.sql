--EXPERIMENT 9--
CREATE OR REPLACE PROCEDURE SP_SP_NAME(IN P_PARAM1 DATATYPE, OUT P_PARAM2 DATATYPE)
LANGUAGE PGPLSQL
AS 
$$
DECLARE
...
BEGIN
	--LOGIC
	SELECT EMPID, ENAME FROM SOMEWHERE
	WHERE EID=V_ID;
END;
$$

EXEC SP_SP_NAME('WOW', NULL)
...
--EXPERIMENT 9--
CREATE OR REPLACE PROCEDURE SP_COUNT_GENDER(IN P_GENDER VARCHAR(10), OUT P_NUM INT)
LANGUAGE PLPGSQL
AS 
$$
BEGIN
	--LOGIC
	SELECT COUNT(ID) INTO P_NUM FROM EMPLOYEE_INFO
	WHERE GENDER=P_GENDER;

	RAISE NOTICE 'THE COUNT OF EMPLOYEE HAVING GENDER AS % IS %', P_GENDER, P_NUM;
END;
$$

call SP_COUNT_GENDER('Male', 0)




CREATE OR REPLACE PROCEDURE SP_GET_ALL_INFO()
LANGUAGE PLPGSQL
AS
$$
	DECLARE 
	...
	BEGIN
		...
		RAISE NOTICE 'THE DETAILS ARE % % %:', V_ID, V_NAME, V_GENDER; 
